...
1. **Multi-agent architecture with differentiated retrieval** — Existing RAG systems rely on content-based retrieval (semantic similarity between query and corpus), applying the same strategy regardless of the downstream task. ATIA introduces task-semantic retrieval: each agent uses a retrieval strategy matched to its function — assertion-based retrieval for claim grounding (RA), graph-based retrieval for prerequisite gap diagnosis (KGA), pattern-based retrieval for question strategy selection (QSA), and content-based retrieval for question generation (GQG). This eliminates the single-retrieval-pipeline bottleneck where one strategy must serve conflicting retrieval needs.

2. **Hybrid knowledge-gap reasoning** — Purely LLM-based knowledge assessment suffers from hallucinated evaluations; classical Knowledge Tracing (BKT, DKT) requires large training datasets unavailable in zero-shot interview settings. ATIA combines symbolic prerequisite graph traversal with neural LLM response interpretation, operating zero-shot without historical interaction data. This enables a novel diagnostic capability — "surface knowledge" detection (candidate demonstrates a topic but lacks its prerequisites) — that neither LLM-only nor KT-only approaches can identify.

3. **Formalized adaptive assessment policy** — Existing LLM interview systems select next questions via opaque prompting, making decisions non-reproducible and non-ablatable. ATIA's Question Strategy Agent operates on an explicit state-action policy with defined state space S, action space A = {PROBE, DRILL, PIVOT, VERIFY, EXPLORE}, and deterministic decision function π(S) → A, formally grounded as a POMDP approximation. This enables full interpretability (every decision traceable to a rule) and clean ablation (policy can be swapped with random/static/learned alternatives).

4. **Profile-based evaluation framework with bias mitigation** — Evaluating adaptive interview systems typically requires costly human studies with small sample sizes. ATIA introduces a profile-based simulated benchmark where ground truth is deterministically derivable from candidate profiles, enabling reproducible comparison at scale (270 interviews × 9 systems). A 3-layer bias mitigation strategy — cross-model compliance validation (GPT-4o-mini checks Claude-based simulator), expert validation (3 evaluators, Krippendorff's alpha), and deterministic ground truth — addresses circular evaluation concerns inherent in LLM-based evaluation.
